...
mr_result_t ecc_import_public(const uint8_t* otherpublickey, uint32_t otherpublickeysize, ecc_point *pub)
{
	...
	FAILIF(result != 0, E_INVALIDOP, "result != 0")
...
{
	FAILIF(!key || !data, E_INVALIDARGUMENT, "!key || !data")
	FAILIF(spaceavail < 32, E_INVALIDSIZE, "spaceavail < 32")

	int len = mp_unsigned_bin_size((mp_int*)&key->k);
	FAILIF(len < 0 || (uint32_t)len > 32, E_INVALIDSIZE, "len < 0 || (uint32_t)len > 32")
...
{
	FAILIF(!key || !digest || !signature, E_INVALIDARGUMENT, "!key || !digest || !signature")
...
{
	FAILIF(!key || !digest || !signature || !signaturesize, E_INVALIDARGUMENT, "!key || !digest || !signature || !signaturesize")
...
{
	FAILIF(!key || !publickey, E_INVALIDARGUMENT, "!key || !publickey")
