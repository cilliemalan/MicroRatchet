...
mr_result_t mr_poly_init(mr_poly_ctx _ctx, const uint8_t* key, uint32_t keysize, const uint8_t* iv, uint32_t ivsize)
{
	...
	FAILIF(keysize != 32, E_INVALIDSIZE, "keysize != 32")
	FAILIF(ivsize != 16, E_INVALIDSIZE, "ivsize != 16")
	FAILIF(!key || !_ctx || !iv, E_INVALIDARGUMENT, "!key || !_ctx || !iv")
...
{
	_mr_poly_ctx* ctx = _ctx;
	FAILIF(!amount, E_INVALIDSIZE, "!amount")
	FAILIF(!data || !_ctx, E_INVALIDARGUMENT, "!data || !_ctx")
...
{
	_mr_poly_ctx* ctx = _ctx;
	FAILIF(!spaceavail, E_INVALIDSIZE, "!spaceavail")
	FAILIF(!output || !_ctx, E_INVALIDARGUMENT, "!output || !_ctx")
