...
mr_result mr_poly_init(mr_poly_ctx _ctx, const uint8_t* key, uint32_t keysize, const uint8_t* iv, uint32_t ivsize)
{
    ...
    AES_KEY aes;
    int r = AES_set_encrypt_key(key + 16, 16 * 8, &aes);
    FAILIF(r, MR_E_INVALIDOP, "Failed to init AES during poly1305 init");
    AES_encrypt(iv, tkey + 16, &aes);
    Poly1305_Init(&ctx->poly, tkey);
...
mr_result mr_poly_process(mr_poly_ctx _ctx, const uint8_t* data, uint32_t amount)
{
    ...
    Poly1305_Update(&ctx->poly, data, amount);
...
// the below is copied from openssl because poly1305 is not directly available
// in the public openssl interface (not on all platforms anyway).
...
void Poly1305_Update(POLY1305* ctx, const unsigned char* inp, size_t len)
{
    ...
    if ((num = ctx->num)) {
        ...
        if (len >= rem) {
            mr_memcpy(ctx->data + num, inp, rem);
            ...
            mr_memcpy(ctx->data + num, inp, len);
            ctx->num = num + len;
            return;
        }
    }
