...
TEST(SymmetricRatchet, Initialize) {
	...
	uint8_t zeroes[KEY_SIZE]{};
	EXPECT_EQ(0U, chain.generation);
	EXPECT_BUFFEREQ(ck, sizeof(ck), chain.chainkey, sizeof(chain.chainkey));
	EXPECT_EQ(0U, chain.oldgeneration);
...
TEST(SymmetricRatchet, MultiSend) {
	...
	uint8_t key1[MSG_KEY_SIZE]{};
	uint8_t key2[MSG_KEY_SIZE]{};
	uint32_t gen1;
	uint32_t gen2;
...
TEST(SymmetricRatchet, BasicReceive) {
	...
	uint8_t zeroes[MSG_KEY_SIZE]{};
	uint8_t zeroes32[KEY_SIZE]{};
	ASSERT_BUFFERNE(key, sizeof(key), zeroes, sizeof(zeroes));

	EXPECT_BUFFERNE(ck, sizeof(ck), chain.chainkey, sizeof(chain.chainkey));
	EXPECT_EQ(1U, chain.generation);
	EXPECT_EQ(0U, chain.oldgeneration);
...
TEST(SymmetricRatchet, SkipReceive) {
	...
	EXPECT_EQ(chain.generation, 10U);
	EXPECT_EQ(chain.oldgeneration, 1U);
...
TEST(SymmetricRatchet, OooReceive) {
	...
	uint8_t key1[MSG_KEY_SIZE]{};
	uint8_t key2[MSG_KEY_SIZE]{};
	uint8_t key3[MSG_KEY_SIZE]{};
	uint8_t key4[MSG_KEY_SIZE]{};
	r = chain_ratchetforreceiving(ctx, &chain, 1, key1, sizeof(key1));
	ASSERT_EQ(MR_E_SUCCESS, r);
	r = chain_ratchetforreceiving(ctx, &chain, 10, key2, sizeof(key2));
	ASSERT_EQ(MR_E_SUCCESS, r);
	...
	EXPECT_EQ(chain.generation, 10U);
	EXPECT_EQ(chain.oldgeneration, 2U);
...
TEST(SymmetricRatchet, MultiSymmetry) {
	...
	uint32_t gen1;
	uint8_t keya1[MSG_KEY_SIZE]{};
	uint8_t keyb1[MSG_KEY_SIZE]{};
	uint32_t gen2;
	uint8_t keya2[MSG_KEY_SIZE]{};
	uint8_t keyb2[MSG_KEY_SIZE]{};
	uint8_t zeroes[MSG_KEY_SIZE]{};
...
void receivetest(uint32_t depth)
{
	mr_ctx ctx = mr_ctx_create(&_cfg);
	mr_rng_ctx rng = mr_rng_create(ctx);
...
void testratchet(const uint8_t chainkey[], size_t chainkeysize,
	uint32_t generation,
	const uint8_t expectedkey[], size_t expectedkeysize)
{
	...
	ASSERT_EQ(MR_E_SUCCESS, r);
	ASSERT_BUFFEREQ(key, sizeof(key), expectedkey, (uint32_t)expectedkeysize);
